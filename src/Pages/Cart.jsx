...
<td><button onClick={()=>dispatch(handleDecrement(product))} className=' btn btn-sm my-3 btn-danger rounded fw-bold text-danger '>-</button></td>
<td  className='fw-bold fs-5 p-2 text-danger'>  {product?.quantity}</td>
<td><button onClick={()=>dispatch(incQuantity(product))} className='btn my-3 btn-success btn-sm rounded fw-bold text-success'>+</button></td> 
</div>
<td>
<td onClick={()=>dispatch(removeCartItem(product?.id))} className='p-2'><i className='fa-solid fa-lg fa-trash text-danger'></i></td>
  ...
  <Link to={'/'} className="btn btn-warning bg-warning text-light">Shop Here</Link>
</div>
...
<i class="fa-regular fa-face-frown fa-2xl"></i>
    ...
    <h2 className='my-3 text-dark'>Your Cart is empty</h2>
  </div>
} 
